...
public class Picture extends SimplePicture 
{
  ...
  public void fixUnderwater() {
      ...
      for (Pixel[] rowArray : pixels) {
          for (Pixel pixelObj : rowArray) {
              ...
              if(pixelObj.getRed()<30 && pixelObj.getBlue()>155){
  ...
  public void mirrorVerticalRightToLeft()
  {
    ...
    for (int row = 0; row < pixels.length; row++)
    {
      for (int col = pixels[0].length-1; col > width / 2; col--)
      {
        ...
        leftPixel = pixels[row][pixels[0].length-col];
  ...
  public void mirrorArms() {
    ...
    {
      // loop from 13 to just before the mirror point
      for (int col = 96; col < mirrorPoint; col++)
      {
        
        leftPixel = pixels[row][col];      
        rightPixel = pixels[204-row+204][col];
        rightPixel.setColor(leftPixel.getColor());
  ...
  */
  ...
  public void copy2(Picture fromPic, int startRow,int endRow, int startCol, int endCol)
  {
    ...
    Pixel[][] toPixels = this.getPixels2D();
    Pixel[][] fromPixels = fromPic.getPixels2D();
    for (int fromRow = startRow, toRow = endRow; fromRow <  fromPixels.length && toRow < endRow;fromRow++, toRow++)
    {
      ...
      {
        fromPixel = fromPixels[fromRow][fromCol];
        toPixel = toPixels[toRow + fromRow][toCol + fromCol];
        toPixel.setColor(fromPixel.getColor());
      }
    }   
  }
  ...
  public void myCollage() {
      ...
      this.copy(moon, 300, 50);
      Picture butterflyNoBlue = new Picture(butterfly);
      butterflyNoBlue.zeroBlue();
      arch.keepOnlyBlue();
      Picture moonNoRed = new Picture(moon);
      //moonNoRed.zeroRed();
      
      this.mirrorVertical();
      this.copy(butterflyNoBlue, 0, 0);
      this.copy(arch, 150, 0);
  ...
  public void edgeDetection2(int colDiff){
    Pixel leftPixel = null;
    Pixel rightPixel = null;
    Pixel[][] pixels = this.getPixels2D();
    Color rightColor = null;
    for (int row = 0; row < pixels.length; row++)
    {
           ...
           col < pixels[0].length-1; col++)
      {
        ...
        rightPixel = pixels[row][col+1];
        rightColor = rightPixel.getColor();
        if (leftPixel.getAverage()-(rightPixel.getAverage())>colDiff)
          leftPixel.setColor(Color.BLACK);
        else {
          leftPixel.setColor(Color.WHITE);
        }
